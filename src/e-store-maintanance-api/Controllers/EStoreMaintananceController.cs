...
{
   ProductRepository productRepository = new ProductRepository("server=127.0.0.1;port=3306;database=e_store_maintanance;user=vicky;password=Dev2024");
   ...
   public IActionResult GetProduct(int id)
   {
     ...
     var productServiceContract = Convert(product);
   ...
   public IActionResult GetProductListOfSupplierId(string supplierId)
   {
      ...
      var productListOfSupplierId= productRepository.GetProductListOfSupplierId(supplierId);
      ...
      foreach(var product in productListOfSupplierId)
          productServiceContract.Add(Convert(product));   
      return Ok(productServiceContract);
   } 
   ...
   public IActionResult AddProduct(ProductServiceContract productServiceContract)
   {
      ...
      {
        var product = ConvertProduct(productServiceContract);
